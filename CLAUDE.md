...
- Floor plan UI: `FloorMap` + `FloorLegend` in `src/components/floor/FloorMap.tsx` (pure SVG from the plan; classes `is-free|taken|unfit|locked|selected|own`; `onSelect(table, state)` leaves the decision to the parent), `useFloorPlan(at)` (keyed by day, topics reservations+content), styles in `src/styles/floor.css`. The booking form's ASZTAL step, the staff booking book (assign select + ALAPRAJZ panel, clashes computed from the list itself) and the showcase JSON editor all use it.
