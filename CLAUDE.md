...
- The House: `MembersPage` at `/staff/members` (manager+), `HouseLookup` (member card, remembers code+phone in localStorage `rm-house-card`, `?code=` prefills) on `VipPage`, the booking form takes `memberCode` (prefilled from `?member=` or the stored card). Reservation `tier` is read-only on the client.
- The House card: `src/lib/houseCard.ts` (tier metal, `ladderOf`, `currentSince`, `CARD_W/H`), `MemberCard` in `src/components/house/MemberCard.tsx` — a still SVG (front: name, code, seal, engraving; back: perks, the ladder with dates) whose colours are attributes, so the PNG is the same drawing; only the sheen and the code letters move via CSS. `CardStage.tsx`: `CardScene` (tilt, flip, plays `issue|upgrade|downgrade|suspend|restore|remove` driven by a `cue` `{play, fromTier, key}`, particles on one canvas), `CardActions` (download/share/flip; `cardImage.ts` rasterises the SVG in the browser with the Google fonts embedded as data URIs — no server, no Cloudinary), `CardOverlay` (the console's stage; `MembersPage` opens it on grant, tier change, suspend/restore, delete, and from the roster thumbnail). `members.tier_history` jsonb (migration 0009) appends on grant and every tier change; `tierHistory` is on both the console and guest views. Styles in `src/styles/cards.css`.
