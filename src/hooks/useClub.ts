...
export interface ClubChatMessage {
  ...
  name: string;
  text: string;
  kind: 'chat' | 'dj' | 'request' | 'request-accepted' | 'request-declined' | 'system' | string;
  color: string;
 ...
 *
 * Chat lines arrive as increments and are applied at once, so the room feels
 * instant; everything else (names, requests, playback) triggers a refetch of
 * the full state. Polling continues underneath as a safety net.
 */
export function useClub(url = '/api/club/state', enabled = true) {
  const {data, refresh, mutate, error} = useLiveData<{state: ClubState}>(url, {intervalMs: 6000, enabled, topics: ['club']});
