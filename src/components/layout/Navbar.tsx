...
import {ChevronDown, Disc3, Menu, X} from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const isDj = !!user?.capabilities.dj;
  ...
  return (
    <header className="fixed left-0 top-0 z-50 flex h-[68px] w-full items-center justify-between border-b border-white/[0.06] bg-[#050304]/80 px-6 backdrop-blur-md lg:px-12">
      <Link to="/" className="flex items-center gap-3 font-heading text-base font-bold tracking-widest text-white">
        <img
          src={assetUrl('/assets/red-moon-logo.png')}
          alt="Red Moon"
      ...
      <nav className="hidden items-center gap-5 md:flex xl:gap-7">
        ...
        <Link to={RESERVE_LINK.to} className="rm-btn is-red !px-3.5 !py-2.5 !text-[9px]">
          {RESERVE_LINK.label}
        </Link>
        ...
        {isDj && (
          <NavLink
            to="/dj"
            title="DJ pult"
            className={({isActive}) => `rm-btn is-ghost !px-3 !py-2.5 !text-[9px]${isActive ? ' is-active' : ''}`}
          >
            <Disc3 size={13}/> DJ
          </NavLink>
        )}

        <Link to={consoleTarget} className="rm-btn is-ghost !px-3.5 !py-2.5 !text-[9px]">
        ...
        createPortal(
          <div className="rm-sheet md:hidden">
            <div className="flex flex-col gap-1 px-6 py-7">
              ...
              <span className="rm-sheet-row rm-label mt-7" style={{animationDelay: `${NAV_LINKS.length * 45}ms`}}>
                {NAV_GROUP.label}
              </span>
              ...
              >
                <Link to={RESERVE_LINK.to} className="rm-btn is-red justify-center !py-3.5 !text-[10px]">
                  {RESERVE_LINK.label}
                </Link>
                {isDj && (
                  <Link to="/dj" className="rm-btn justify-center !py-3.5 !text-[10px]">
                    <Disc3 size={13}/> DJ PULT ↗
                  </Link>
                )}
                <Link to={consoleTarget} className="rm-btn is-ghost justify-center !py-3.5 !text-[10px]">
